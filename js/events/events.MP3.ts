...
(Gate as any)._write3 = function(dataView: DataView, event: IGateEvent, info: IEventWriteInfo, temp: any) {
  ...
  if (info.boardIndex === 0) {
    ...
    if (curGate === 1) {
      ...
      // 31F97A    6
      // 31F982    R0 -> A2
      sceneView.setUint16(0x446A, event.gatePrevChain[0]); // 0x80109E08, 0x0031F97A
      sceneView.setUint32(0x4470, makeInst("ADDIU", REG.A2, REG.R0, event.gatePrevChain[1])); // 0x80109E10, 0x0031F980

      // Entering space
      sceneView.setUint16(base + 0x62, event.gateEntryIndex); // 0x80109B38
      sceneView.setUint16(base + 0xEE, event.gateEntryIndex); // 0x80109BC4
      sceneView.setUint16(base + 0x22A, event.gateEntryIndex); // 0x80109D00
      sceneView.setUint16(base + 0x326, event.gateEntryIndex); // 0x80109DFC
      ...
      sceneView.setUint32(0x4344, makeInst("ADDIU", REG.V0, REG.R0, event.gateEntryIndex)); // 0x80109CE4, 0x0031F854
      sceneView.setUint32(0x4348, makeInst("BEQ", REG.S1, REG.V0, 0x0005 << 2)); // 0x80109CE8 0x0031F858 // Keep branch to 0x80109D00
      // There's two!
      sceneView.setUint32(0x4440, makeInst("ADDIU", REG.V0, REG.R0, event.gateEntryIndex)); // 0x80109DE0, 0x0031F950
      sceneView.setUint32(0x4444, makeInst("BEQ", REG.S1, REG.V0, 0x0005 << 2)); // 0x80109DE4 0x0031F954 // Keep branch to 0x80109DFC

      // Gate space
      sceneView.setUint16(base + 0x106, event.gateSpaceIndex); // 0x80109BDC

      // Exit space
      sceneView.setUint16(base + 0x7A, event.gateExitIndex); // 0x80109B50
      sceneView.setUint16(base + 0xE6, event.gateExitIndex); // 0x80109BBC
      sceneView.setUint16(base + 0x216, event.gateExitIndex); // 0x80109CEC
      sceneView.setUint16(base + 0x312, event.gateExitIndex); // 0x80109DE8

      // Next space
      // after 6D gate
      // 31F962    6
      // 31F996    4
      sceneView.setUint16(0x4452, event.gateNextChain[0]); // 0x0031F962
      sceneView.setUint16(0x4486, event.gateNextChain[1]); // 0x0031F996
    }
    else if (curGate === 2) {
      // Previous space
      // pre 93 gate
      // 31F99A    9
      // 31F9A2    2
      sceneView.setUint16(0x448A, event.gatePrevChain[0]); // 0x80109E28, 0x0031F99A
      sceneView.setUint16(0x4492, event.gatePrevChain[1]); // 0x80109E30, 0x0031F9A2

      // Entering space
      sceneView.setUint16(base + 0x6A, event.gateEntryIndex);
      sceneView.setUint16(base + 0x206, event.gateEntryIndex);
      sceneView.setUint16(base + 0x2FE, event.gateEntryIndex);

      // Gate space
      sceneView.setUint16(base + 0x10E, event.gateSpaceIndex);

      // Exit space
      sceneView.setUint16(base + 0x92, event.gateExitIndex);

      // Next space
      // after 93 gate
      // 31F96A    9
      // 31f9B6    6
      sceneView.setUint16(0x445A, event.gateNextChain[0]); // 0x0031F96A
      sceneView.setUint16(0x44A6, event.gateNextChain[1]); // 0x0031F9B6
      // after 93 gate also
      // 31F9B2    9
      // 31F9B6    6
      sceneView.setUint16(0x44A2, event.gateNextChain[0]); // 0x0031F9B2
    }
