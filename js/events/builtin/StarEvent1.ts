...
export const Star1Event = createEvent("STAR1", "Buy star");
Star1Event.activationType = EventActivationType.WALKOVER;
Star1Event.executionType = EventExecutionType.DIRECT;
Star1Event.fakeEvent = true;
Star1Event.supportedGames = [
  Game.MP1_USA,
];
Star1Event.parse = function(dataView: DataView, info: IEventParseInfo) {
  let hashes = {
    // DK, Wario, Luigi
...
Star1Event.write = function(dataView: DataView, event: IEvent, info: IEventWriteInfo, temp: any) {
  return `
    addiu SP, SP, -0x18
    sw    RA, 0x10(SP)
