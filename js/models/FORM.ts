  ...
  export const FORM = class FORM {
    static unpack(formView: ArrayBuffer | DataView): IFormObj | null {
      ...
      while (curOffset < formView.byteLength - 2) { // Roughly-ish the end
        ...
        let parsed = PP64.utils.FORM.parseType(type as IFormObjType, entry.raw);
        if (parsed)
          entry.parsed = parsed;
    ...
    static getObjectsByType(form: IFormObj, type: number) {
