...
export function decodeVADPCM(tblView: DataView, outRawData: ArrayBuffer, len: number, book: ALADPCMBook, decode8Only: boolean): number {
  ...
  let samples: number = 0; // int

  // flip the predictors (TODO: What does flip mean?)
  //let predsBuffer = new ArrayBuffer(32 * book.npredictors);
  let preds = new Int16Array(16 * book.npredictors); // new DataView(predsBuffer); // signed short *
  ...
  if (!decode8Only) {
    ...
    while (_len > 0) {
      ...
      let pred1Index = pred * 16; // signed short *

      inIndex++;
      decode8(tblView, inIndex, outRawData, outIndex, index, preds, pred1Index, lastsmp);
      inIndex += 4;
      _len -= 4;
      outIndex += 16;
    ...
    while (_len > 0) {
      ...
      let pred1Index = pred * 16;

      inIndex++;
      decode8Half(tblView, inIndex, outRawData, outIndex, index, preds, pred1Index, lastsmp);
      inIndex += 2;
      _len -= 2;
      outIndex += 16;
...
function decode8(
  ...
  let tmp = new Int16Array(8); // signed short tmp[8];
  ...
  let pred2Index = (pred1Index + 8); // signed short *
  ...
  for (let i = 0; i < 8; i++) {
    ...
    let result = ((tmp[i] << 0xb) + total) >> 0xb;
    if (result > 32767)
      sample = 32767;
...
function signExtend(b: number, x: number) {
  let m = 1 << (b - 1); // mask can be pre-computed if b is fixed
