...
namespace PP64.utils.img {
  ...
  export const ImgPack = new class ImgPack {
    // Rips each image from a ImgPack and returns an array of RGBA32.
    fromPack(buffer: ArrayBuffer) {
      ...
      for (var i = 0; i < imageCount; i++) {
        ...
        if (hasPalette)
          rawImgBuffer = PP64.utils.img.BMP.toRGBA(rawImgBuffer, palette, bitCount, 16);
        //PP64.utils.arrays.print(rawImgBuffer);
        ...
        if (bitCount === 32) {
          ...
          imgInfo.src = PP64.utils.img.RGBA5551.toRGBA32(rawImgBuffer, imgWidth, imgHeight);
        }
        else if (bitCount === 4 || bitCount === 8) {
          imgInfo.src = PP64.utils.img.Grayscale.toRGBA32(rawImgBuffer, imgWidth, imgHeight, bitCount);
        }
        output.push(imgInfo);
        //PP64.utils.arrays.print(output[i]);
    ...
    toPack(imgInfoArr: IImgInfo[], outBpp: number, bmpBpp: number, oldPack: ArrayBuffer) {
      ...
      if (bmpBpp) { // FIXME: There could be multiple images and the palette needs to be comprehensive.
        [bmpData, palette] = PP64.utils.img.BMP.fromRGBA(imgInfoArr[0].src!, imgInfoArr[0].bpp!, 8);
      ...
      let curOffset = 0x20;
      for (let i = 0; i < imgInfoArr.length; i++) {
        curOffset = PP64.utils.img.ImgPack._writeEntry(newView, curOffset, imgInfoArr[i], outBpp, oldView);
      }

      return newPack;
    }

    getByteLength(imgInfoArr: IImgInfo[], outBpp: number) {
    ...
    _writeEntry(newView: DataView, curOffset: number, imgInfo: IImgInfo, outBpp: number, oldView?: DataView) {
      ...
      if (imgInfo._bmpData) {
        ...
        let rgba16 = PP64.utils.img.RGBA5551.fromRGBA32(imgInfo.src!, imgInfo.width, imgInfo.height);
      ...
      PP64.utils.arrays.copyRange(newView, srcView, curOffset, 0, srcView.byteLength);
