namespace PP64.utils.img {
  ...
  export const HVQ = class HVQ {
    static decode(hvqView: DataView) {
    ...
    static _getChunkOffset(hvqView: DataView, chunkIndex: number) {
      // Chunk offsets start at 0x20
      return hvqView.getUint32(0x20 + (chunkIndex * 4));
    }

    static _recursiveHuffmanBuild(hvqView: DataView, bitOffset: number): any {
    ...
    static encode(buffer: ArrayBuffer, width: number, height: number) {
      const gameVersion = PP64.romhandler.getGameVersion();
      if (gameVersion === null)
