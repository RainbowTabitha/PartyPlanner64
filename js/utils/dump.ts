namespace PP64.utils {
  ...
  export const dump = class Dump {
    ...
    static strings(searchStr: string = "", raw: boolean = false) {
    ...
    static saveWaluigi() {
      ...
      for (let f = 0; f < 163; f++) {
        (window as any).waluigiParts.push(PP64.fs.mainfs.get(8, f));
      }
    ...
    static writeWaluigi(character = 1) {
      ...
      for (let i = 0; i <  (window as any).waluigiParts.length; i++) {
        PP64.fs.mainfs.write(character, i,  (window as any).waluigiParts[i]);
      }
    }
    ...
    static saveDaisy() {
      ...
      (window as any).daisyParts.push(PP64.fs.mainfs.get(9, 1));
      (window as any).daisyParts.push(PP64.fs.mainfs.get(9, 3));
    ...
    static writeDaisy(character = 6) {
      ...
      PP64.fs.mainfs.write(character, 158, (window as any).daisyParts[0]);
      PP64.fs.mainfs.write(character, 159, (window as any).daisyParts[1]);
    }

    // Helper for finding FS read locations
    static searchForPatchLocations(offset: number) {
    ...
    static printAsm(start: number, end: number) {
      let romView = PP64.romhandler.getDataView();
      let curOffset = start;
      let insts = [];
      while (curOffset <= end) {
     ...
     */
    static printSceneAsm(sceneIndex: number) {
