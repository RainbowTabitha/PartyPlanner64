...
import { splice } from "../../packages/lib/utils/string";
...
const MPImageComponent = (props: any) => {
  const ch = props.children[0].props.text; // 1337 draft.js h4x
  let className = "mpImage";
  const key = _ToolbarCharToKey[ch];
  className += " mpImageKey-" + key;

  // Stop React warnings from unwanted spreaded props...
  const outerSpanProps = { ...props };
...
const _ToolbarCharToKey: { [char: string]: string } = {};
const _ToolbarKeyToChar: { [char: string]: string } = {};
...
class MPEditorToolbar extends React.Component<MPEditorToolbarProps> {
  ...
  render() {
    ...
    const items = _ToolbarItems.map((itemDef) => {
...
function _getNextStyleTag(rawString: string) {
  ...
  while (startingIndex < rawString.length) {
    const styleStart = rawString.indexOf("<", startingIndex) + 1;
    if (!styleStart)
      // indexOf yielded -1
      return [];

    const styleEnd = rawString.indexOf(">", styleStart);
